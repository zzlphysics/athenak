...
void DynGRMHDPS<EOSPolicy, ErrorPolicy>::QueueDynGRMHDTasks() {
  ...
  pnr->QueueTask(&MHD::EFieldSrc, pmhd, MHD_EFieldSrc, "MHD_EFieldSrc", Task_Run,
                 {MHD_EField});
  pnr->QueueTask(&MHD::SendE, pmhd, MHD_SendE, "MHD_SendE", Task_Run,
                 {MHD_EFieldSrc});
  pnr->QueueTask(&MHD::RecvE, pmhd, MHD_RecvE, "MHD_RecvE", Task_Run, {MHD_SendE});
  pnr->QueueTask(&MHD::ObserveEField, pmhd, MHD_ObserveE, "MHD_ObserveE", Task_Run,
                 {MHD_RecvE});
  pnr->QueueTask(&MHD::CT, pmhd, MHD_CT, "MHD_CT", Task_Run, {MHD_ObserveE});
...
TaskStatus DynGRMHDPS<EOSPolicy, ErrorPolicy>::ConToPrim(Driver *pdrive, int stage) {
  ...
  if (pmy_pack->pmhd->c2p_spatial_telemetry) {
    (void) eos.template ConsToPrim<false, true>(
        pmy_pack->pmhd->u0, pmy_pack->pmhd->b0,
        pmy_pack->pmhd->bcc0, pmy_pack->pmhd->w0, temperature,
        0, n1m1, 0, n2m1, 0, n3m1, false, false, true, stage);
  } else {
...
C2PProjectionStats DynGRMHDPS<EOSPolicy, ErrorPolicy>::ConToPrimBC(
  ...
  if (pmy_pack->pmhd->c2p_spatial_telemetry && count_events) {
    return eos.template ConsToPrim<false, true>(
        pmy_pack->pmhd->u0, pmy_pack->pmhd->b0,
        pmy_pack->pmhd->bcc0, pmy_pack->pmhd->w0, temperature,
        is, ie, js, je, ks, ke, false, preserve_cons, count_events, 0);
  }
