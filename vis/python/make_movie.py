        ...
        if use_mpi:
            ...
            if len(local_files) > 0:
                ...
                for fname in local_files:
                    ...
                    args.output_file = os.path.join(output_dir, fname.replace('.bin', '.png'))
                    main(**vars(args))
            else:
                print(f"Rank {rank} has no files to process.", flush=True)
            ...
            for file in os.listdir(args.data_file):
                if file.endswith('.bin'):
                    print("Imaging :", file)
                    args.data_file = os.path.join(input_dir, file)
                    args.output_file = os.path.join(output_dir, file.replace('.bin', '.png'))
                    main(**vars(args))
    else:
        print("Error: data_file is not a file or a directory")
